...
volatile bit flag_bat_is_full;      // 电池是否被充满电的标志位

volatile bit flag_is_in_charging;               // 是否处于充电的标志位
volatile bit flag_tim_scan_maybe_not_charge;    // 用于给定时器扫描的标志位，可能检测到了拔出充电器
volatile bit flag_tim_scan_maybe_in_charging;   // 用于给定时器扫描的标志位，可能检测到了充电器
volatile bit flag_tim_set_is_in_charging;       // 由定时器置位/复位的，表示是否有插入充电器的标志位
volatile bit flag_tim_scan_bat_maybe_full;      // 用于给定时器扫描的标志位，可能检测到电池被充满电
volatile bit flag_tim_set_bat_is_full;          // 由定时器置位/复位的，表示电池是否被充满电的标志位
volatile bit flag_tim_scan_bat_maybe_near_full; // 用于给定时器扫描的标志位，可能检测到电池快被充满电
volatile bit flag_tim_set_bat_is_near_full;     // 由定时器置位/复位的，表示电池是否快被充满电的标志位

volatile bit flag_tim_scan_maybe_low_bat; // 用于给定时器扫描的标志位，可能检测到了低电量
volatile bit flag_tim_set_bat_is_low;     // 由定时器置位/复位的，表示在工作时，电池是否处于低电量的标志位

// volatile bit flag_tim_scan_maybe_shut_down  ; // 用于给定时器扫描的标志位，可能检测到了 电池电压 低于 关机对应的电压
// volatile bit flag_tim_set_shut_down ;        // 由定时器置位/复位的，表示在工作时，检测到了 电池电压 在一段时间内都低于 关机对应的电压

volatile bit flag_tim_scan_maybe_motor_stalling; // 用于给定时器扫描的标志位，可能检测到了电机堵转
volatile bit flag_tim_set_motor_stalling;        // 由定时器置位/复位的，表示在工作时检测到了电机堵转

volatile bit flag_is_enable_key_scan; // 标志位，是否使能按键扫描(每10ms被定时器置位，在主函数中检测并清零)
// volatile bit flag_is_dev_open ;        // （只在长按开机、关机时使用）设备是否开机的标志位，0--未开机，1--开机

volatile bit flag_ctl_led_blink; // 控制标志位，是否控制指示灯闪烁
volatile bit flag_ctl_turn_dir;  // 控制标志位，是否更换电机的方向

volatile bit flag_ctl_dev_close; // 控制标志位，是否要关闭设备

volatile bit flag_ctl_low_bat_alarm; // 控制标志位，是否使能低电量报警

volatile bit flag_is_disable_to_open; // 标志位，是否不使能开机(低电量不允许开机)

volatile bit flag_is_recv_ctl; // 标志位，是否接收了控制命令
/*
    标志位，是否通过语音来切换方向.
    用在定时器中断中，判断自动换方向，如果这个标志位置一，
    会清除当前自动换方向的计时，重新开始自动换方向的计时,
    定时器清除计时后，会给这个标志位清零
*/
volatile bit flag_is_turn_dir_by_speech;
...
/*
    ...
    定时器在清除计时后，会给这个标志位清零
*/
volatile bit flag_is_new_operation;
